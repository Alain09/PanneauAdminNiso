...
export async function PATCH(
    ...
    try {
        ...
        // les donnees requises
        if (!countOption) {
            return NextResponse.json(
                { message: "nombre d'options requis", success: false },
                { status: 400 }
            );
        }
        ...
        if (!existingOption || existingOption.categoriesStatistiquesPayement.userId !== id) {
            return NextResponse.json(
                { message: "Option non trouvée ou n'appartient pas à cet utilisateur", success: false },
                { status: 404 }
            );
        ...
        const newTotalPaidByWeek = allOptions.reduce((sum, opt) =>
            opt.id === optionId
                ...
                : sum + (opt.totalToPayByWeekOfThisOption ?? 0), 0
        );
